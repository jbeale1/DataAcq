...
/**
  ...
  * @attention
  *
  * Read ADC channels, calculate mean, stdev stats
...
#define CHNUM 2               // how many ADC channels are converted
#define ADC_BUFFER_SIZE 8000  // how many uint16 words in ADC buffer; max ~ 20k
#define DECIMATION (200/CHNUM)  // average this many raw samples (must be integer)
#define PING_BUFFER_SIZE (ADC_BUFFER_SIZE / (2 * CHNUM * DECIMATION))
...
volatile uint32_t ping_buffer[2][CHNUM][PING_BUFFER_SIZE];
...
int main(void)
{
  ...
  // find standard deviation of data samples in buffer[] of length size
  void getStDev(uint32_t *buffer, int size, double * stdevp, double * meanp) {
  ...
  void process_adc_data(unsigned char pp) {
      ...
      // int sMax = 0;
      //int sMin = INT_MAX;
      double mean, stdev;  // to calculate standard deviation


      if (dc_avg < 0) {
    	  dc_avg = ping_buffer[pp][k][PING_BUFFER_SIZE/2];  // initialize very first time
    	  lpval = dc_avg;
      }


      int i=0;
      int bp = 0;  // pointer into output character string buffer
      int ccount;

      if (showRaw) {
		  do {
			  for (k=0;k<CHNUM;k++) {
				  ccount = sprintf(&out_buffer[bp],"%ld\n", (long int)ping_buffer[pp][k][i]);
				  bp += ccount;
				  i++;
			  }
      ...
      uint32_t samples[PING_BUFFER_SIZE];
      if (showStats) {
		  // print out min/max/mean/stdev stats
    	  for (k=0;k<CHNUM;k++) {
    		  for (int a=0;a<PING_BUFFER_SIZE;a++) samples[a]=ping_buffer[pp][k][a];
			  getStDev(samples, PING_BUFFER_SIZE, &stdev, &mean);
			  ccount = sprintf(&out_buffer[bp],"%5.3f,%5.3f, ", mean, stdev);
			  bp += ccount;
    	  }
      }
	  ccount = sprintf(&out_buffer[bp],"\n");
	  bp += ccount;
    ...
    if (pingReady[p]) {
		...
		for(int i=0; i<PING_BUFFER_SIZE; i++) {
			...
			for (int j=0; j<DECIMATION; j++) {
				for (int k=0; k<CHNUM; k++) {
					sum[k] += adc_buffer[i*DECIMATION*CHNUM + j*CHNUM + k];  // first half of buffer
				}
			}
			for (int k=0; k<CHNUM; k++) {
				ping_buffer[p][k][i] = sum[k];
			}
		}
		...
		for(int i=0; i<PING_BUFFER_SIZE; i++) {
			...
			for (int j=0; j<DECIMATION; j++) {
				for (int k=0; k<CHNUM; k++) {
					...
					sum[k] += adc_buffer[(i+PING_BUFFER_SIZE)*DECIMATION*CHNUM + j*CHNUM + k];
				}
			}
			for (int k=0; k<CHNUM; k++) {
				ping_buffer[p][k][i] = sum[k];
