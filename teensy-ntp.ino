...
#define WAIT_COUNT 3
// subtract this # seconds from GPS time to get Unix time (0 = Jan.1 1970)
#define GPS2UNIX (2208988800)

float cpuTfilt; // LP-filtered CPU temperature
float Tf = 0.01; // filtering constant for CPU temperature;
...
void setup() {
  ...
  cpuTfilt=0.0;
  const int TAvg1 = 20;
  for (int i=0;i<TAvg1;i++) {
    cpuTfilt += tempmonGetTemp();  // initialize filtered version of CPU temperature
  }
  cpuTfilt /= TAvg1;
...
void updateTime(uint32_t gpstime) {
  ...
  if(settime) {
    ...
    if(ClockPID.full() && wait) {
      ...
      cpuTfilt = (1.0-Tf)*cpuTfilt + Tf*tempmonGetTemp();
      Serial.print(samples[median_index].pps);
      Serial.print(" ");
      Serial.print(offsetHuman, 9);
      Serial.print(" ");
      Serial.print(ClockPID.d(), 10);  // frequency drift
      Serial.print(" ");
      Serial.print(ClockPID.d_chi(), 9);
      Serial.print(" ");
      Serial.print(localClock.getPpb());
      Serial.print(" ");
      uint32_t ut = samples[median_index].gpstime - (uint32_t) GPS2UNIX;
      Serial.print(ut); // display Unix C time (Jan.1.1970 epoch)
      Serial.print(" ");
      Serial.print( cpuTfilt,2 ); // Teensy 4.1 CPU temperature, deg.C
      Serial.println();
    }
  } else {
