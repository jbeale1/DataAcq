...
ADS1256 A(0, 1, 2, 3, 2.500); //DRDY, RESET, SYNC(PDWN), CS, VREF(float).    // RP2040 Standard (JPB)

// Pi Pico SPI defaults: GPIO16 (RX), GPIO17 (CS), GPIO18 (SCK), and GPIO19 (TX)
  ...
  if (Serial.available() > 0)
  {
    ...
    switch (commandCharacter) //based on the command character, we decide what to do
    {
      ...
      case 'G': //Read a single input continuously
        ...
        {
          Serial.println(A.convertToVoltage(A.readSingleContinuous()), 8);
          //The conversion is printed in Volts with n decimal digits
          //Note: Certain serial terminals cannot keep up with high speed datastream!
        }
        A.stopConversion();
        break;
      //--------------------------------------------------------------------------------------------------------
      case 'H': //Read a single input continuously and get stats on N samples (H20 = groups of 20)
        {
          ...
          float x,mean,delta,m2,variance,stdev;  // to calculate standard deviation 
          ...
          while (Serial.read() != 's') // stopped by a character from the serial port
          {
              ...
              sMax = -1E20;  // set max to minimum possible reading
              sMin = 1E20;  // set min to max possible reading
              n = 0;    // have not made any ADC readings yet
              mean = 0; // start off with running mean at zero
              m2 = 0;
                      
              for (int i=0;i<sampleCount;i++) {
                x = A.convertToVoltage(A.readSingleContinuous());  // analog reading in volts
                x *= 1E6;  // change units from volts to microvolts
                datSum += x;
                if (x > sMax) sMax = x;
                if (x < sMin) sMin = x;
              ...
              stdev = sqrt(variance);  // Calculate standard deviation in microvolts
              Serial.print(mean,3);
              Serial.print(", ");
      ...
      case 'a': //Testing a single conversion - Only one single result is returned
        ...
        break;
      //--------------------------------------------------------------------------------------------------------
      case 'b': //Testing N single conversions - N results returned (b10 = 10 readings)
        {
          while (!Serial.available());
          int readCount = Serial.parseInt(); //Parse the number
      ...
      case 'M': //set MUX
        {
          ...
          if (inputMode == 's') //single-ended
          {
            ...
            Serial.print("Single-ended channel: ");
          ...
          if (inputMode == 'd') //differential
          {
            while (!Serial.available());
            inputChannel = Serial.parseInt();
            Serial.print("Differential channel pair: ");
            Serial.println(inputChannel);
      ...
      case 'P': //Set PGA
        {
          ...
          Serial.print(pgaSelection);
          Serial.print("  code: ");
          Serial.println(pgaValues[pgaSelection]);
