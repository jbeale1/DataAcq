...
char fout[] = "dht-log.csv";    // log measured data to this output file
...
int main( void ) {
    ...
    FILE *of = fopen(fout,"w");  // open data log file for writing

    char hstring[] = "date_time, epoch, degC, RH\n";
    printf(hstring);
    fprintf(of, hstring);
    int lines = 0;  // how many lines printed
    ...
    for (;; ) // infinite loop
    {
        ...
        unsigned int epoch = (unsigned)time(NULL);
        printf("%04d-%02d-%02d_%02d:%02d:%02d, %u, ", 
           year,month,day,hours,minutes,seconds, epoch);
        fprintf(of, "%04d-%02d-%02d_%02d:%02d:%02d, %u, ", 
           year,month,day,hours,minutes,seconds, epoch);
